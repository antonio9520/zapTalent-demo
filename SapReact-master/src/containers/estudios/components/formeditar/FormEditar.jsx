...
const Formulario = (props) => {
  ...
  const [file, setFile] = useState(
    data ? (data.estudioURL ? data.estudioURL : null) : null
  );

  //data
  const [idusuario] = useState(usuario._id);
  const [tipoestudio, setTipoEstudio] = useState(
    data ? data.tipoestudio : null
  );
  const [carrera, setCarrera] = useState(data ? data.carrera : null);
  const [institucion, setInstitucion] = useState(
    data ? data.institucion : null
  );
  const [areaestudio, setAreaEstudio] = useState(
    data ? data.areaestudio : null
  );
  const [_id] = useState(data ? data._id : null);
  const [escalanotas, setEscalaNotas] = useState(
    data ? data.escalanotas : null
  );
  const [promedio, setPromedio] = useState(data ? data.promedio : null);
  const [pais, setPais] = useState(data ? data.pais : null);
  const [observacion, setObservacion] = useState(
    data ? data.observacion : null
  );
  const [active, setActive] = useState(data ? data.estado : null);
  const [diainicio, setValueI] = useState(data ? data.diainicio : null);
  const [diafin, setValueT] = useState(data ? data.diafin : null);
  ...
  return (
    <div className="cont-form-estudios">
      ...
      <div className="top-form-estudios">
        <p className="p1-form-estudios">Editar estudio</p>
      </div>
          ...
          <div className="div-2">
            {inputText ? (
              ...
              <CustomInput
                ...
                name="promedio"
                defaultValue={promedio}
                value={promedio ? promedio.toUpperCase() : null}
                helpertext={promediomsg}
                error={promedioError}
                // inputProps={{ maxLength: "1" }}
              />
